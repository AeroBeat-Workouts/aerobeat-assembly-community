...
The prior upstream branding caveat has now been resolved by the follow-up input-core release refresh. After the follow-up release-consumer refresh moved the assembly manifest pin from `v0.1.1` to `v0.1.2`, removed the stale mounted/cache copies for `aerobeat-input-core`, and reran `godotenv addons install`, the refreshed installed dependency payload now also reports the corrected plugin-internal version truthfully: `addons/aerobeat-input-core/plugin.cfg:5` now reads `version="0.1.2"` while preserving the already-correct renamed addon identity (`addons/aerobeat-input-core/README.md:1` = `# aerobeat-input-core`, `plugin.cfg:2-3` = `name="AeroBeat Input Core"` plus description `"Shared input abstractions and provider contracts for the AeroBeat ecosystem"`). The refreshed install evidence is the assembly manifest line `8:      "checkout": "v0.1.2",`, `git show v0.1.2:plugin.cfg` reporting `5:version="0.1.2"`, and the installed payload at `addons/aerobeat-input-core/plugin.cfg` also reporting `5:version="0.1.2"`.
